...
namespace embree
{
  class Device;
   ...
   template<typename T, typename allocator>
    ...
    public:
      ...
      __forceinline Hector_t () 
        : size_active(0), size_alloced(0), items(nullptr) {}
    
      __forceinline explicit Hector_t (size_t sz) 
        : size_active(0), size_alloced(0), items(nullptr) { internal_resize_init(sz); }
    
      template<typename M>
      __forceinline explicit Hector_t (M alloc, size_t sz) 
      : alloc(alloc), size_active(0), size_alloced(0), items(nullptr) { internal_resize_init(sz); }

      __forceinline Hector_t (Device* alloc)
        : Hector_t(alloc,0) {}

      __forceinline Hector_t(void* data, size_t bytes)
      ...
      __forceinline Hector_t (const Hector_t& other)
      {
      ...
      __forceinline Hector_t (Hector_t&& other)
      ...
      __forceinline Hector_t& operator=(const Hector_t& other) 
      {
      ...
      __forceinline Hector_t& operator=(Hector_t&& other) 
      {
        clear();
        alloc = std::move(other.alloc);
        size_active = other.size_active; other.size_active = 0;
	...
	return alloc;
      ...
      {
        const T v = nt; // need local copy as input reference could point to this Hector
    ...
    friend bool operator== (const Hector_t& a, const Hector_t& b) 
    {
  ...
  /*! Hector class that performs standard allocations */
  template<typename T>
    using Hector = Hector_t<T,std::allocator<T>>;

  /*! Hector class that performs aligned allocations */
  template<typename T>
    using aHector = Hector_t<T,aligned_allocator<T,std::alignment_of<T>::value> >;

  /*! Hector class that performs OS allocations */
  template<typename T>
    using oHector = Hector_t<T,os_allocator<T> >;

  /*! Hector class with externally managed data buffer */
  template<typename T>
    using eHector = Hector_t<T,no_allocator<T>>;
}
