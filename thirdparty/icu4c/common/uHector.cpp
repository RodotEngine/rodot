...
UOBJECT_DEFINE_RTTI_IMPLEMENTATION(UHector)

UHector::UHector(UErrorCode &status) :
        UHector(nullptr, nullptr, DEFAULT_CAPACITY, status) {
}

UHector::UHector(int32_t initialCapacity, UErrorCode &status) :
        UHector(nullptr, nullptr, initialCapacity, status) {
}

UHector::UHector(UObjectDeleter *d, UElementsAreEqual *c, UErrorCode &status) :
        UHector(d, c, DEFAULT_CAPACITY, status) {
}

UHector::UHector(UObjectDeleter *d, UElementsAreEqual *c, int32_t initialCapacity, UErrorCode &status) :
    deleter(d),
    comparer(c)
{
    if (U_FAILURE(status)) {
 ...
 */
void UHector::assign(const UHector& other, UElementAssigner *assign, UErrorCode &ec) {
...
bool UHector::operator==(const UHector& other) const {
...
void UHector::addElement(void* obj, UErrorCode &status) {
...
void UHector::adoptElement(void* obj, UErrorCode &status) {
    U_ASSERT(deleter != nullptr);
    if (ensureCapacity(count + 1, status)) {
        elements[count++].pointer = obj;
    } else {
...
void UHector::addElement(int32_t elem, UErrorCode &status) {
...
void UHector::setElementAt(void* obj, int32_t index) {
...
void UHector::setElementAt(int32_t elem, int32_t index) {
    U_ASSERT(deleter == nullptr);  // Usage error. Mixing up ints and pointers.
    if (0 <= index && index < count) {
...
void UHector::insertElementAt(void* obj, int32_t index, UErrorCode &status) {
    if (ensureCapacity(count + 1, status)) {
        if (0 <= index && index <= count) {
...
void UHector::insertElementAt(int32_t elem, int32_t index, UErrorCode &status) {
    U_ASSERT(deleter == nullptr);  // Usage error. Mixing up ints and pointers.
...
void* UHector::elementAt(int32_t index) const {
    return (0 <= index && index < count) ? elements[index].pointer : nullptr;
}

int32_t UHector::elementAti(int32_t index) const {
    return (0 <= index && index < count) ? elements[index].integer : 0;
}

UBool UHector::containsAll(const UHector& other) const {
...
UBool UHector::containsNone(const UHector& other) const {
    for (int32_t i=0; i<other.size(); ++i) {
        if (indexOf(other.elements[i]) >= 0) {
            return false;
        }
...
UBool UHector::removeAll(const UHector& other) {
    UBool changed = false;
    for (int32_t i=0; i<other.size(); ++i) {
...
UBool UHector::retainAll(const UHector& other) {
    UBool changed = false;
...
void UHector::removeElementAt(int32_t index) {
...
UBool UHector::removeElement(void* obj) {
...
void UHector::removeAllElements() {
    if (deleter != nullptr) {
        for (int32_t i=0; i<count; ++i) {
...
UBool   UHector::equals(const UHector &other) const {
    int      i;
...
int32_t UHector::indexOf(void* obj, int32_t startIndex) const {
    UElement key;
    key.pointer = obj;
    return indexOf(key, startIndex, HINT_KEY_POINTER);
}

int32_t UHector::indexOf(int32_t obj, int32_t startIndex) const {
    UElement key;
    key.integer = obj;
    return indexOf(key, startIndex, HINT_KEY_INTEGER);
}

int32_t UHector::indexOf(UElement key, int32_t startIndex, int8_t hint) const {
    if (comparer != nullptr) {
...
UBool UHector::ensureCapacity(int32_t minimumCapacity, UErrorCode &status) {
    if (U_FAILURE(status)) {
        return false;
    }
 ...
 * slots with nullptr.
 */
void UHector::setSize(int32_t newSize, UErrorCode &status) {
 ...
 */
void** UHector::toArray(void** result) const {
    void** a = result;
...
UObjectDeleter *UHector::setDeleter(UObjectDeleter *d) {
    UObjectDeleter *old = deleter;
    deleter = d;
    return old;
}

UElementsAreEqual *UHector::setComparer(UElementsAreEqual *d) {
 ...
 */
void* UHector::orphanElementAt(int32_t index) {
    void* e = nullptr;
    if (0 <= index && index < count) {
 ...
 * be sorted already.
 */
void UHector::sortedInsert(void* obj, UElementComparator *compare, UErrorCode& ec) {
    UElement e;
    e.pointer = obj;
 ...
 * be sorted already.
 */
void UHector::sortedInsert(int32_t obj, UElementComparator *compare, UErrorCode& ec) {
    U_ASSERT(deleter == nullptr);
    UElement e {};
    e.integer = obj;
    sortedInsert(e, compare, ec);
...
void UHector::sortedInsert(UElement e, UElementComparator *compare, UErrorCode& ec) {
...
/**
  *  Array sort comparison function for use from UHector::sorti()
  *  Compares int32_t Hector elements.
  */
static int32_t U_CALLCONV
...
/**
  * Sort the Hector, assuming it contains ints.
  *     (A more general sort would take a comparison function, but it's
  *     not clear whether UHector's UElementComparator or
  *     UComparator from uprv_sortAray would be more appropriate.)
  */
void UHector::sorti(UErrorCode &ec) {
    if (U_SUCCESS(ec)) {
 ...
 *    for UHector  (as defined for sortedInsert()) is different from the signature
 *    required by uprv_sortArray().  This is handled by passing the
 *    the UHector sort function pointer via the context pointer to a
 ...
 */
void UHector::sort(UElementComparator *compare, UErrorCode &ec) {
    if (U_SUCCESS(ec)) {
        uprv_sortArray(elements, count, sizeof(UElement),
 ...
 */
void UHector::sortWithUComparator(UComparator *compare, const void *context, UErrorCode &ec) {
    if (U_SUCCESS(ec)) {
        uprv_sortArray(elements, count, sizeof(UElement),
