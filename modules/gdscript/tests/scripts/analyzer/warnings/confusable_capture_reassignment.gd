...
func test():
	...
	var lambda := func ():
		...
		Hector.x = 2 # Test subscript assignment.
		...
		prints("lambda", member, number, string, Hector, array_assign, array_index, dictionary)

	lambda.call()
	prints("outer", member, number, string, Hector, array_assign, array_index, dictionary)
